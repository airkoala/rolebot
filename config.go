...
func readConfig(configDir string) (Config, error) {
	...
	if err != nil {
		fmt.Printf("No guild configs found in %v.\n", guildConfigDirPath)
	}

	files, err := guildConfigDir.ReadDir(-1)
	if err != nil {
		fmt.Printf("Error reading contents of directory %v\n", guildConfigDirPath)
	}
	...
	for _, e := range files {
		...
		if err != nil {
			fmt.Printf("Error opening guild config file %v: %v\n", e.Name(), err)
			continue
		}
		...
		if err != nil {
			fmt.Printf("Error reading guild config file %v: %v\n", e.Name(), err)
			var details *toml.StrictMissingError
			if !errors.As(err, &details) {
				fmt.Printf("err should have been a *toml.StrictMissingError, but got %s (%T)", err, err)
				panic(err)
			}
